...
name: monitoring
description: >
  元认知监控子模块。在 Plan active 阶段提供偏差预防提醒和自我监控指引。
  核心原则：插件提供目标和历史偏差信息，Agent 自主监控本轮输出，主动标记偏差。
version: 4.0.0
...
本 skill 在 **`before_prompt_build`**（task.status === `"active"`）时注入。
...
2. 了解历史偏差（避免重复）
3. 本轮输出后自行对比目标
4. 若发现偏差，输出 `[STATUS: revising] [REASON: xxx]`
...
2. **偏差判定**
   ...
   | **重大偏差** | 影响阶段目标或整体 Plan | **输出 `[STATUS: revising] [REASON: xxx]`** |

3. **偏差记录**（重大偏差时）
   - 在当前任务事件文件（`.openclaw/events/{YYYY-MM-DD}/{HH-MM-SS}.md`）中追加「偏差」章节
   - 记录偏差类型、描述、影响范围
...
- [ ] 历史偏差中有无类似问题需要注意？
- [ ] 本轮输出是否符合当前阶段的预期范围？
- [ ] 若发现重大偏差，是否已输出 `[STATUS: revising] [REASON: xxx]`？
...
active（Plan 执行中）
    ...
    ├─ 无偏差 → 继续执行
    ├─ 轻微偏差 → 自行调整
    ├─ 重大偏差 → 输出 [STATUS: revising] [REASON: xxx]
    │               ↓
