...
│
├── .agents                # 元数据层（隐藏目录）
│   ├── events/            # 事件流：.agentsevents/{YYYY-MM-DD}/{runId}.md
│   │   └── archive/       # 归档事件：.agentsevents/archive/{date}-{runId}.md
│   ├── locks/             # 并发控制：.agentslocks/{文件路径替换-为-}.json
│   ├── decisions/         # 决策存档：.agentsdecisions/{YYYYMMDD-HHMM}-标题.md
│   └── tasks/             # 任务索引：.agentstasks/{runId}.json
...
```bash
mkdir -p .agents{events,locks,decisions,tasks}
