...
> **版本**：v4.5.0
> **更新**：移除系统级键（State/Memory/Flow/Log），保留项目级文件系统映射；Hook 改为条件触发机制

v4.5.0 是 **Tool + Hook Plugin**，无系统层 SQLite、无 `ctx.state` 注入。所有数据持久化通过项目级文件系统完成。
...
| 键 | 触发时机 | 条件 | 注入内容 |
|----|----------|------|----------|
| `reg:task-created` | `task.create` 成功后 | after_tool_call | 制定计划提醒 |
| `reg:deviation-detected` | `task.update` 含 deviation | after_tool_call | 偏差分析提醒 |
| `reg:event-generated` | `event.report` 成功后 | after_tool_call | 六维度平衡性判断 |
| `reg:no-active-task` | 无 active task | before_prompt_build | 创建任务提醒 |
...
v4.3.0 扁平化重构后（v4.5.0 保持），以下系统级存储不再存在：
...
**注意**：Hook 机制（v4.5.0）不再依赖系统级存储，使用项目级文件系统 + idempotencyKey 去重。
...
所有时间戳使用 **ISO-8601** 字符串格式（如 `2026-05-22T10:00:00.000Z`），不再使用 Unix 毫秒时间戳。
