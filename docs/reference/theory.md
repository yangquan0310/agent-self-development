...
任务完成后，插件将 `.agent/tasks/{runId}.json` 中的计划、偏差和归因整合为事件，形成按时间序列组织的自传体记忆：
...
任务完成后：
  → 插件整合：计划 + 偏差 + 归因 → 事件文件
  → 事件按时间序列存入 .agent/events/{YYYY-MM-DD}/{HH-MM-SS}.md
...
|----------|----------|-----------|-----------|
| 自传体记忆 | 以"我"为索引的行动记忆 | 事件文件系统 | `.agent/events/` |
| 工作自我 | 执行控制系统 | 任务状态管理 | `.agent/tasks/{runId}.json` |
...
| 分布式记忆 | 外部辅助记忆设备 | 外部文件系统 | `.agent/` + 项目文件 |
