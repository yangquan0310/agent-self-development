...
- **触发时机**：v4.5.0 采用 `after_tool_call` 条件触发机制，在关键业务节点精准提醒
  - `task.create` 成功后 → M1 提醒（制定计划）
  - `task.update` 含 deviation → M2 提醒（偏差分析）
  - `event.report` 成功后 → M3 提醒（六维度平衡性判断）
  - 无 active task 时（`before_prompt_build`）→ M4 提醒（创建任务）
...
> 本节提供皮亚杰认知发展理论的范畴论形式化，作为理论的数学深度补充。
...
| 范畴元素 | 定义 | Agent 对应 |
|----------|------|------------|
| $\text{Ob}(\mathbf{Cog})$ | 认知图式（Schema）集合 | Agent 的 SOUL.md、IDENTITY.md、skills/ 中的能力定义 |
| $\text{Hom}(A, B)$ | 从图式 $A$ 到 $B$ 的认知态射 | 同化/顺应/平衡化操作 |
| $\circ$ | 态射复合（认知操作的组合） | 连续任务执行形成的策略链 |
| $\text{id}_A$ | 单位态射（图式自我保持） | 无操作，状态维持 |

### 10.2 态射类型的形式化

设 $A, B \in \text{Ob}(\mathbf{Cog})$：

**同化态射（Assimilation）**：

$$
f_{\text{assim}}: A \to A \quad \text{当且仅当} \quad \exists \text{新信息 } x \text{ 使得 } x \text{ 可被 } A \text{ 容纳}
$$

同化的范畴论性质：
- **闭合性**：$f_{\text{assim}}$ 的输出仍在 $\text{Ob}(\mathbf{Cog})$ 中
- **自映射**：定义域 = 值域 = $A$（结构不变）

**顺应态射（Accommodation）**：

$$
f_{\text{acco}}: A \to B \quad \text{当且仅当} \quad \nexists \text{新信息 } x \text{ 使得 } x \text{ 可被 } A \text{ 容纳}
$$

和 $B$ 是 $A$ 的结构扩展（$B$ 的表示能力 > $A$）

**平衡化态射（Equilibration）**：

$$
\epsilon: A \to A' \quad \text{其中} \quad A' = \text{通过同化/顺应恢复平衡的图式}
$$
...
$$
F_{ij}: \mathcal{S}_i \to \mathcal{S}_j \quad (i \neq j)
$$

例如：
- $F_{1,6}: \mathcal{S}_{\text{ego}} \to \mathcal{S}_{\text{skill}}$ — 自我认知影响技能习得
- $F_{4,3}: \mathcal{S}_{\text{identity}} \to \mathcal{S}_{\text{belief}}$ — 身份影响信念
...
$$
\vec{s}(t+1) = \vec{s}(t) + \Delta \vec{s}(t)
$$

其中状态增量：

$$
\Delta \vec{s}(t) = \sum_{i=1}^{6} \alpha_i \cdot \text{Switch}(I_i(e_t), \text{op}_i) \cdot \vec{e}_i
$$

- $I_i(e_t)$：事件 $e_t$ 在维度 $i$ 的失衡信号
- $\text{op}_i \in \{\text{ASSIM}, \text{ACCOM}, \text{NO_OP}\}$
- $\vec{e}_i$：第 $i$ 维度的单位向量
- $\alpha_i$：维度 $i$ 的学习率

**吸引子分析**：

定义**发展吸引子** $A \subset \mathcal{A}$：

$$
A = \{ \vec{s} \in \mathcal{A} : \Phi(\vec{s}) = \vec{s} \}
$$
...
| 理论概念 | 数学表示 | 范畴论对应 |
|----------|----------|------------|
| 认知图式 | $S \in \mathcal{S}_i$ | 对象 $A \in \text{Ob}(\mathbf{Cog})$ |
| 同化 | $f_{\text{assim}}: A \to A$ | 自态射 |
| 顺应 | $f_{\text{acco}}: A \to B$ | 态射 $A \to B, B \neq A$ |
| 平衡化 | $\epsilon: A \to A'$ | 平衡化态射 |
| 平衡态 | $\vec{s}^* \in \mathcal{A}, V(\vec{s}^*) = 0$ | 吸引子 |
| 六维度状态 | $\vec{s} \in \mathcal{S}_1 \times \cdots \times \mathcal{S}_6$ | 状态向量 |
| 失衡度量 | $V(\vec{s}) = 1 - \mathcal{B}(\vec{s})$ | 李雅普诺夫函数 |

---

*文档版本：v1.2.1 | 范畴论框架已整合*
