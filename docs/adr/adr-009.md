...
- `event.md` 是项目级文件（`.agentsevents/...`），不是系统级 state
...
| 类 | 职责 | 存储位置 |
|----|------|----------|
| **EventObject** | 项目级 `event.md` 的创建、追加、查询、归档 | `.agentsevents/*.md` |
| **MemoryArchive**（原 Event 类改名） | 系统级 EventLog 的归档和查询 | `~/.agentsmemory/{agentId}.sqlite` |
