...
### v4.5.0：Hook 注入时机精细化 ✅ 已完成（2026-05-26）
...
| 里程碑 | 时间 | 交付物 | 状态 |
|--------|------|--------|------|
| M1 | 2026-05-26 | `after_tool_call` + M1/M3 条件触发 | ✅ 已完成 |
| M2 | 2026-05-26 | `after_tool_call` + M2 条件触发 | ✅ 已完成 |
| M3 | 2026-05-26 | `before_prompt_build` + M4 条件降频 | ✅ 已完成 |
| M4 | 2026-05-26 | 测试覆盖 + 文档更新 | ✅ 已完成 |
...
- [x] **P0-4：测试覆盖 + 文档更新** — Developer
  - 来源问题：Hook 注入需要完整测试覆盖
  - 交付物：
    - `test/hooks-conditional.test.js`（新增）：4 个触发场景的单元测试（17 个用例全部通过）
    - 更新 `SKILL.md`：新增 Hook 时机矩阵说明
  - 验收标准：4 个触发场景全部测试通过 ✅
  - 里程碑：M4 🔖 commit: `feat: P0-4 测试覆盖 + 文档更新`
