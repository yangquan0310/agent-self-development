...
## 已识别问题（PM 讨论中沉淀，待后续版本处理）

| # | 问题 | 严重度 | 说明 | 建议处理版本 |
|---|------|--------|------|-------------|
| D | 运行时技能目录命名职责不清 | 🟡 低 | `skill/` 命名与 OpenClaw 生态"skill = 可安装扩展包"惯用法冲突；开发者难以区分运行时指导文本 vs 项目级技能模板 | v4.1.x 补丁或 v4.2.0 |

**问题 D 详情**：
- **表现**：`skill/` 目录存放 Agent 运行时通过 Tool 读取的指导文本，但"skill"一词在生态中表示"可安装的扩展包/工具集"
- **根因**：v4.1.0 将 `src/**/SKILL.md` 移出代码目录时，仅用单复数区分，未做语义分离
- **影响**：命名债务，可能导致开发者/Agent 理解混淆
- **候选方案**：`guides/`、`prompts/`、`directives/`、`playbooks/`
- **验收标准**：命名无歧义、文档明确标注职责边界、测试通过
